class DateRangePicker {
  ...
  static #handleClickBtnNav(instance, date, calendar, direction) {
    ...
    DateRangePicker.#generateBodyTable(
      instance,
      calendar.querySelector(".cal-body-table"),
      date.getFullYear(),
      date.getMonth()
    );
  }

  static #handleClickBtnSelectYear(
    inputYear,
    instance,
    tableContainer,
    yearSpan
    ...
    if (/[0-9]{4,5}/.test(inputYear.value)) {
      ...
      const yearSelectorContainer = tableContainer.querySelector(
        ".cal-year-selector-container"
      );
  ...
  static #handleQuickSelectClick(instance, tableContainer) {
    ...
    if (tableDay) tableDay.classList.add("cal-hide");
    ...
    const btnSelectYear = tableContainer.querySelector(
      "button.cal-btn-select-year"
    );
    ...
    const btnNavContainer =
      tableContainer.parentElement.querySelector(".cal-nav-container");
    btnSelectYear.onclick = (e) => {
      DateRangePicker.#handleClickBtnSelectYear(
        inputYear,
        instance,
        tableContainer,
        yearSpan
  ...
  static #generateYearSelector(instance, tableContainer) {
    ...
    yearSelectorContainer.setAttribute(
      "class",
      "cal-year-selector-container cal-hide"
    );
    ...
    for (let i = currentYear - 15; i <= currentYear + 9; i++) {
      ...
      if (i === currentYear) td.classList.add("cal-current-year");
      td.textContent = i;
      td.dataset.year = i;
      tr.appendChild(td);
      j++;
    ...
    tbody.onclick = (e) => {
      ...
      if (target.nodeName.toLowerCase() === "td") {
        const yearSpan =
          tableContainer.parentElement.querySelector(".cal-year");
        ...
        yearSelectorContainer.classList.add("cal-hide");
        const tableMonth = tableContainer.querySelector(".cal-table-month");
        if (tableMonth) tableMonth.classList.remove("cal-hide");
      }
    };
  ...
  static #generateMonthSelector(instance, tableContainer) {
    ...
    for (let i = 0; i < DateRangePicker.#fullnameMonth.length; i++) {
      ...
      if (i === currentMonth) td.classList.add("cal-current-month");
    ...
    tbody.onclick = (e) => {
      ...
      if (target.nodeName.toLowerCase() === "td") {
        const monthSpan =
          tableContainer.parentElement.querySelector(".cal-month");
  ...
  static #generateBodyTable(instance, tbody, year, month) {
    ...
    for (let i = 0; i < nbDayThisMonth; i++) {
      ...
      if (
        i + 1 === DateRangePicker.currentDate.getDate() &&
      ...
      if (
        DateRangePicker.#formatDate(instance.start) ===
        DateRangePicker.#formatDate(tmpDate)
      )
        td.classList.add("cal-start");
      if (
        DateRangePicker.#formatDate(instance.end) ===
        DateRangePicker.#formatDate(tmpDate)
      )
  ...
  #dateSelected = new Date(
    DateRangePicker.currentDate.getFullYear(),
    DateRangePicker.currentDate.getMonth(),
    1
  );
  #target = null;
  #callback = null;
  ...
  constructor(
    ...
    if (
      ...
      throw new Error(
        "onchange doit être de type Function et ses deux premiers arguments doivent être des dates (en string ou un objet Date)."
      );
    }
    ...
    this.#target.value = `${DateRangePicker.#formatDate(
      this.#start
    )} - ${DateRangePicker.#formatDate(this.#end)}`;
  ...
  #init() {
    ...
    const tableDay = DateRangePicker.#generateTable(
      this,
    ...
    calSelectContainer.innerHTML = `
            <span class="cal-month" data-month="${this.#dateSelected.getMonth()}">${
      DateRangePicker.#month[this.#dateSelected.getMonth()]
    }</span>&nbsp;&nbsp;
    ...
    btnNavPrev.onclick = (e) => {
      DateRangePicker.#handleClickBtnNav(
        this,
        this.#dateSelected,
        this.#calendar,
        "prev"
    ...
    const tableMonth = DateRangePicker.#generateMonthSelector(
      this,
      tableContainer
    );
    const tableYear = DateRangePicker.#generateYearSelector(
      this,
      tableContainer
    );
  ...
  #stepZero(target) {
    ...
    const tdsDateInRange =
      this.#calendar.querySelectorAll(`td.cal-date-in-range`);
    tdsDateInRange.forEach((td) => td.classList.remove("cal-date-in-range"));
    const tdEnd = document.querySelector(".cal-end");
    if (tdEnd) tdEnd.classList.remove("cal-end");
    const tdStart = document.querySelector("td.cal-start");
    if (tdStart) tdStart.classList.remove("cal-start");
  ...
  #stepOne(target) {
    ...
    try {
      ...
      if (tmpDate >= this.#start) this.#end = tmpDate;
      else {
    ...
    document
      .querySelectorAll(".cal-date-in-range")
      .forEach((td) => td.classList.remove("cal-date-in-range"));
    ...
    if (this.#callback && this.#args)
      this.#callbackReturn = this.#callback.apply(null, [
        this.#start,
        this.#end,
        ...this.#args,
      ]);
